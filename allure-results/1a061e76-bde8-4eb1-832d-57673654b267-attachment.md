...
- Name: AccountRegistration.spec.ts >> User should register a new account successfully
- Location: tests\AccountRegistration.spec.ts:6:5

# Error details

```
Error: locator.click: Error: strict mode violation: getByText('My Account') resolved to 3 elements:
    ...
    3) <a href="https://naveenautomationlabs.com/opencart/index.php?route=account/account">My Account</a> aka getByRole('link', { name: 'My Account', exact: true })
...
```ts
  ...
  15 |        this.page=page;
  16 | 
  17 |        this.btnSearch= page.getByRole('button',{name: ''});
  18 |        this.lnkLogin = page.getByText('Login');
  19 |        this.lnkRegister = page.getByText('Register');
  20 |        this.lnkMyAccount = page.getByText('My Account');
  21 |        this.txtSearchbox = page.getByText('Search');
  22 |     }
  23 |     
  24 | async  clickMyAccount(){
> 25 |      await this.lnkMyAccount.click();
     |                              ^ Error: locator.click: Error: strict mode violation: getByText('My Account') resolved to 3 elements:
  26 | }
  27 | 
  28 | async clickLogin(){
  29 |     await this.lnkLogin.click()
  30 | }
  31 | 
  32 | async searchProduct(product: string){
  33 |        await this.txtSearchbox.fill(product);
  34 |         await this.btnSearch.click();
  35 | }
  36 | 
  37 | async clickRegisterLnk(){
